/*
       ____   ____    _     
   ___/ ___| / ___|  (_)___   ConnectedBusiness -> Sales Order Module
  / _ \___ \| |      | / __|  ---------------------------------------
 |  __/___) | |___ _ | \__ \  |- Handles so lookup / insertion
  \___|____/ \____(_)/ |___/  |- Creates so, so detail and so payment
                   |__/       
...
// Locates newly posted invoices for processing
function pollCB(callback){
	var findNewlyPosted=new Request("SELECT i.InvoiceCode, i.MerchantOrderID_DEV000221, s.TrackingNumber, s.CarrierDescription, d.ItemName, d.ShipQuantity, z.SourcePurchaseID_DEV000221 FROM acdd.dbo.CustomerInvoice i LEFT OUTER JOIN acdd.dbo.Shipment s ON s.ConvertedSourceDocument = i.InvoiceCode LEFT OUTER JOIN acdd.dbo.ShipmentDetail d ON d.TrackingNumber = s.TrackingNumber LEFT OUTER JOIN acdd.dbo.CustomerInvoiceDetail z ON (z.InvoiceCode = i.InvoiceCode AND z.LineNum = d.LineNum) WHERE i.WebSiteCode='WEB-000002' AND i.InvoiceCode LIKE 'INV-%' AND i.DatePosted > '"+config.lastUpdate+"'",function(err,rowCount,invRows){
		if (err){
			log.doLog('error','pollCB','Error polling CB: '+err);
		}
		
		if (rowCount>0){
			// Combine
			var invoices={}
			for (var i in invRows){
				var inv=invRows[i];
				
				// Invoice undefined?
				if (invoices[inv.MerchantOrderID_DEV000221.value]==undefined){
					invoices[inv.MerchantOrderID_DEV000221.value]={
						invoiceCode: inv.InvoiceCode.value,
						tracks: {}
					}
				}
				
				// Tracking number undefined?
				if (invoices[inv.MerchantOrderID_DEV000221.value].tracks[inv.TrackingNumber.value]==undefined && inv.TrackingNumber.value!=null){
					invoices[inv.MerchantOrderID_DEV000221.value].tracks[inv.TrackingNumber.value]={
						carrier: (inv.CarrierDescription.value).toLowerCase(),
						items: {}
					}
				}
				
				// SKU undefined?
				if (inv.ItemName.value!=null){
					if (invoices[inv.MerchantOrderID_DEV000221.value].tracks[inv.TrackingNumber.value].items[inv.ItemName.value]==undefined){
						invoices[inv.MerchantOrderID_DEV000221.value].tracks[inv.TrackingNumber.value].items[inv.ItemName.value]={
							itemId: inv.SourcePurchaseID_DEV000221.value,
							shipQuant: inv.ShipQuantity.value
						}
					}else{
						invoices[inv.MerchantOrderID_DEV000221.value].tracks[inv.TrackingNumber.value].items[inv.ItemName.value].shipQuant+=inv.ShipQuantity.value;
					}
				}
			}
	...
	connection.execSqlBatch(findNewlyPosted);
}
	...
	lookupSKUS(skuList,function(itemInfo){
		...
		for (var i in items){
			...
			if (itemInfo[item.sku]==undefined){
				...
				var sodTmp=templates.salesOrderDetail(soCode,parseInt(i)+1,item,itemInfo[item.sku],so);
