/*
       ____   ____    _     
   ___/ ___| / ___|  (_)___   ConnectedBusiness -> MSSQL Templates
  / _ \___ \| |      | / __|  ------------------------------------
 |  __/___) | |___ _ | \__ \  |- JSON templates for MSSQL insertion
  \___|____/ \____(_)/ |___/  |- Helper functions for templates
                   |__/       
...
exports.salesOrder=function(so,soCode,cust,bill,ship){
	return {
		...
		PODate: so.created_at,
		WarehouseCode: 'MAIN',
		ShippingMethodGroup: 'DEFAULT',
		ShippingMethodCode: translateFreightCode(so.shipping_method),
		ShippingDate: so.created_at,
		LatestShipDate: '2079-06-05 00:00:00',
		DueDate: so.created_at,
		CancelDate: so.created_at,
		...
		DateShipped: so.created_at,
		RootOrderCode: soCode,
		SaveCounterID: 1,
...
exports.salesOrderDetail=function(soCode,lineNum,item,itemInfo,so){
	return {
		SalesOrderCode: soCode,
		...
		UserModified: 'eSC',
		DueDate: so.created_at,
		CommissionApplyTo: 'Sales',
		DateCreated: so.created_at,
		DateModified: so.created_at,
		SortOrder: lineNum,
		CouponDiscount: 0.00,
		CouponDiscountRate: 0.00,
		SerializeLot: 'False',
...
function translateFreightCode(code){
	var freight;
	switch (code){
		case 'fedex_FEDEX_2_DAY':freight="FedEx 2 Day";break;
		case 'fedex_FEDEX_EXPRESS_SAVER':freight="FedEx Express Saver";break;
		case 'fedex_PRIORITY_OVERNIGHT':freight="FedEx Priority Overnight";break;
		case 'fedex_STANDARD_OVERNIGHT':freight="FedEx Standard Overnight";break;
		case 'freeshipping_freeshipping':freight="FedEx Ground";break;
		case 'ups_01':freight="Next Day";break;
		case 'ups_02':freight="2nd Day";break;
		case 'ups_03':freight="Ground";break;
		case 'ups_12':freight="Three Day";break;
		default:freight="FedEx Ground";break;
	}
